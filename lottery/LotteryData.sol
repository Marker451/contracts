...
contract LotteryData is Ownable {
	...
	struct LotteryConfig {
		...
		uint commission;        //手续费总额 单位：Wei
		uint awardPerCopy;      //每份奖金金额  单位：Wei
		uint seed;              //随机数种子
		uint commissionRatio;    //手续费比例手续费比例手续费比例 10 表示10% 
		address commissionReceiver;  //手续费接收地址
	}
	...
	struct LotteryResult {
		...
		uint    totalCommission;                   //总的手续费 单位：Wei
	...
	function joinLottery(uint lotteryID, address msgSender, uint msgValue)
		onlyControl payable public
		{
			...
			uint _commission = msgValue.mul(lottery.config.commissionRatio).div(100);
			uint _val = msgValue.sub(_commission);

			lottery.config.totalAward += _val;
			lottery.config.commission += _commission;
		...
		returns(bool)
		{
			...
			if (lottery.status == LotteryStatus.Started) {
				if (lottery.mode == LotteryMode.Sum || lottery.mode == LotteryMode.Address) {
					...
					if (now >= config.openCondition) {
						if(lottery.joiner.indexs.length >= 10 ){
							lottery.status = LotteryStatus.ToOpen;
						}else{
							refund = true;
						}
	...
	function _drawLottery(Lottery storage lottery)  internal {
		...
		LotteryResult storage result = lottery.result;
		result.totalCommission = lottery.config.commission;

		if(lottery.config.commissionReceiver != address(0) && lottery.config.commission != 0){
			lottery.config.commissionReceiver.send(lottery.config.commission);
		}
	...
	function setLotteryConfig(
			...
			uint openCondition, uint copies, uint commissionRatio,
		...
		onlyControl public
		{
			...
			require(commissionRatio >=0 && commissionRatio <=100);
